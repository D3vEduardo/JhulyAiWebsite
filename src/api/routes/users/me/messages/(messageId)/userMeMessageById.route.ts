...
const log = debug("app:api:users:me:messages:messageId");

export const usersMeMessageByIdRoute = new Hono().use("*", authMiddleware).get(
  "/",
  zValidator(
    "param",
    z.object({
  ...
  async (c) => {
    ...
    try {
      ...
      const message = await prisma.message.findUnique({
        where: {
          id: param.messageId,
          senderId: user.id, // Garantir que a mensagem pertence ao usuário autenticado
        },
      ...
      if (!message) {
        log(
          `[GET] /api/users/me/messages/${param.messageId}: Message not found for user ${user.id}`
        );
        return c.json(
          {
      ...
      log(
        `[GET] /api/users/me/messages/${param.messageId}: Message fetched successfully for user ${user.id}`
      );
      return c.json({
      ...
      log(
        `[GET] /api/users/me/messages/${param.messageId}: Error for user ${user.id}:`,
        error
      );
