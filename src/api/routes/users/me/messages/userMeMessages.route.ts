...
export const usersMeMessagesRoute = new Hono().use("*", authMiddleware).get(
  ...
  async (c) => {
    ...
    try {
      ...
      log(
        `[GET] /api/users/me/messages: Retrieved ${result.length} messages for user ${user.id}`
      );

      return c.json({
        message: "Success on get user messages!",
        data: result,
        pagination: {
          nextCursor,
          hasNextPage,
      ...
      log(`[GET] /api/users/me/messages: Error for user ${user.id}:`, error);
