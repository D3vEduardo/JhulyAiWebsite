...
const log = debug("app:api:users:me:chats:chatId");

export const usersMeChatByIdRoute = new Hono().use("*", authMiddleware).get(
  "/",
  zValidator(
    "param",
    z.object({
  ...
  zValidator("query", chatSelectQuerySchema.strict()),
  async (c) => {
    ...
    try {
      ...
      const chat = await prisma.chat.findUnique({
        where: {
          id: param.chatId,
          ownerId: user.id, // Garantir que o chat pertence ao usuário autenticado
        },
      ...
      if (!chat) {
        log(
          `[GET] /api/users/me/chats/${param.chatId}: Chat not found for user ${user.id}`
        );
        return c.json(
          {
      ...
      log(
        `[GET] /api/users/me/chats/${param.chatId}: Chat fetched successfully for user ${user.id}`
      );
      return c.json({
      ...
      log(
        `[GET] /api/users/me/chats/${param.chatId}: Error for user ${user.id}:`,
        error
      );
